...
for(i=0,j=0;j<keylen;j++,i+=4){k[j]=key.charCodeAt(i)|(key.charCodeAt(i+1)<<8)|(key.charCodeAt(i+2)<<16)|(key.charCodeAt(i+3)<<24);}
for(j=kc-1;j>=0;j--){tk[j]=k[j];}
r=0;t=0;for(j=0;(j<kc)&&(r<rounds+1);){for(;(j<kc)&&(t<4);j++,t++){keySched[r][t]=tk[j];}
if(t==4){r++;t=0;}}
while(r<rounds+1){var temp=tk[kc-1];tk[0]^=S[B1(temp)]|(S[B2(temp)]<<8)|(S[B3(temp)]<<16)|(S[B0(temp)]<<24);tk[0]^=Rcon[rconpointer++];if(kc!=8){for(j=1;j<kc;j++)tk[j]^=tk[j-1];}else{for(j=1;j<kc/2;j++){tk[j]^=tk[j-1];}
temp=tk[kc/2-1];tk[kc/2]^=S[B0(temp)]|(S[B1(temp)]<<8)|(S[B2(temp)]<<16)|(S[B3(temp)]<<24);for(j=kc/2+1;j<kc;j++){tk[j]^=tk[j-1];}}
...
function byteArrayToHex(byteArray){var result="";if(!byteArray)return;for(var i=0;i<byteArray.length;i++){result+=((byteArray[i]<16)?"0":"")+byteArray[i].toString(16);}
return result;}
function hex2s(hex){var r='';if(hex.indexOf("0x")==0||hex.indexOf("0X")==0){hex=hex.substr(2);}
if(hex.length%2){hex+='0';}
for(var i=0;i<hex.length;i+=2){r+=String.fromCharCode(parseInt(hex.slice(i,i+2),16));}
return r;}
function formatPlaintext(plaintext){var bpb=blockSizeInBits/8;var i;if(typeof plaintext=="string"||plaintext.indexOf){plaintext=plaintext.split("");for(i=0;i<plaintext.length;i++){plaintext[i]=plaintext[i].charCodeAt(0)&0xFF;}}
for(i=bpb-(plaintext.length%bpb);i>0&&i<bpb;i--){plaintext[plaintext.length]=0;}
return plaintext;}
function getRandomBytes(howMany){var i;var bytes=new Array();for(i=0;i<howMany;i++){bytes[i]=Math.round(Math.random()*255);}
return bytes;}
var blockSizeInBits=128;var keySizeInBits=256;function rijndaelEncrypt(plaintext,key,mode){var i,aBlock;var bpb=blockSizeInBits/8;var ct;if(!plaintext||!key)return;if(key.length*8!=keySizeInBits)return;if(mode=="CBC"){ct=getRandomBytes(bpb);}else{mode="ECB";ct=new Array();}
plaintext=formatPlaintext(plaintext);var expandedKey=new keyExpansion(key);for(var block=0;block<plaintext.length/bpb;block++){aBlock=plaintext.slice(block*bpb,(block+1)*bpb);if(mode=="CBC"){for(var i=0;i<bpb;i++){aBlock[i]^=ct[block*bpb+i];}}
ct=ct.concat(AESEncrypt(aBlock,expandedKey));}
return ct;}
function rijndaelDecrypt(ciphertext,key,mode){var bpb=blockSizeInBits/8;var pt=new Array();var aBlock;var block;if(!ciphertext||!key)return;if(typeof ciphertext=="string"){ciphertext=ciphertext.split("");for(i=0;i<ciphertext.length;i++){ciphertext[i]=ciphertext[i].charCodeAt(0)&0xFF;}}
if(key.length*8!=keySizeInBits)return;if(!mode){mode="ECB";}
var expandedKey=new prepareDecryption(key);for(block=(ciphertext.length/bpb)-1;block>0;block--){aBlock=AESDecrypt(ciphertext.slice(block*bpb,(block+1)*bpb),expandedKey);if(mode=="CBC"){for(var i=0;i<bpb;i++){pt[(block-1)*bpb+i]=aBlock[i]^ciphertext[(block-1)*bpb+i];}}else{pt=aBlock.concat(pt);}}
if(mode=="ECB"){pt=AESDecrypt(ciphertext.slice(0,bpb),expandedKey).concat(pt);}
return pt;}